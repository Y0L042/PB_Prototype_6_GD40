...
#	var isRaycastIntersected = do_raycast(self.get_global_position(), raycast_dir * RAY_LENGTH, [self.get_rid()]).empty()
#	while isRaycastIntersected:
#		offset_raycast_dir = raycast_dir.rotated(raycast_rotation_offset)
#		isRaycastIntersected = do_raycast(self.get_global_position(), self.get_global_position() + offset_raycast_dir * RAY_LENGTH, [self.get_rid()]).empty()
